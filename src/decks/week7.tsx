...
export const week7 = {
  ...
  slides: [
    ...
    <SplitMediaSlide
      title="Some patterns only make sense in motion"
      points={[
        <>
          Feedback, loading, and transitions are about <strong>time</strong>. A screenshot can't show them
        </>,
        <>
          When a check is about <strong>behavior</strong> (a field validating, a skeleton resolving, an undo), show a short clip, not a still
        </>,
        <>
          In your audit, <Highlight>watch the motion</Highlight>, not just the frames. That's where most flows fall apart
        </>,
      ]}
      media={{
        src: "/refs/skeleton-loading-demo.mp4",
        poster: "/refs/skeleton-loading-finance.png",
        caption: "A skeleton resolving into content (drop a clip or paste a URL to play it live)",
        credit: "Isaac Sanchez, Dribbble",
        href: "https://dribbble.com/shots/12105359-Skeleton-Loading-for-a-Finance-App",
      }}
    />,
  ...
  notes: [
    // 1. Title
    "Quick framing. Today is a working session, not a lecture. We sharpen what you already have, we don't build anything new. By the end you'll have rebuilt one part of your product using two real audit methods.",
    // 2. Well that was a ride
    "A breather. You've been building for weeks, so take a second to acknowledge that. Then point forward: today is about making the work tighter, not bigger.",
    // 3. Agenda
    "Two passes and a build. First you audit your own design system, then you swap and audit another pair's flow, then you fix one real thing. Both passes are timeboxed, so we keep moving.",
    // 4. How we work today
    "Set the critique norms before anyone gives a note. This is where workshops live or die. Critique the design, not the person. A note is a specific observation plus a suggestion, not 'I don't like it.' Plus, don't only minus. Everyone gives a note, and the pair being reviewed listens first, then decides what to act on.",
    // 5. Two passes diagram
    "Walk the map once so nobody is lost. Pass 1 is solo, on your own system, 15 minutes. Pass 2 is with another pair, on their flow, 20 minutes. The output is the whole point: one real change in Figma.",
    // 6. Activity 1: inventory
    "This is Brad Frost's interface inventory. Reassure them it's fast: it's all in Figma already, you're just pulling every instance of each component onto one board. Grouping like with like is what makes the inconsistencies jump out.",
    // 7. Gallery: create an inventory
    "The PNC example always lands: dozens of button styles on one real site. Then show the fix beside it, one button with every state defined once. That before and after is what you want them to feel about their own file. Worth naming: B2B products have the same problem, a dashboard like Linear or Stripe hides just as many one-off buttons. The audit is the same whoever the user is.",
    // 8. Checklist: audit the design system
    "Hand them the checklist and let them work, you're circulating now, not presenting. Point at the 60-30-10 callback and the contrast numbers, those are the ones they forget.",
    // 9. Activity 2: review another pair's flow
    "Nielsen's heuristics on a real flow. The trick is walking it as a first-time user and saying out loud what you expect at each step. Severity 0 to 4 forces them to prioritize instead of dumping a flat list.",
    // 10. Checklist: audit the design patterns
    "This is the behavior checklist, not the looks one. Feedback and status is where most student flows fail: no loading, no confirmation. Empty and error states are the next biggest gap.",
    // 11. Gallery: what good looks like
    "Three of the checks, done well in real products. Use these as the bar when you audit. Empty states that offer a next step, inline validation, a skeleton on load.",
    // 12. Motion
    "A point about your audit: static frames hide the thing that matters most in these patterns, which is time. When a check is about behavior, watch the motion. This is also why the deck plays clips and gifs, so we can show an interaction instead of describing it.",
    // 13. Divider: share what changed
    "Regroup. Two or three pairs show one before and after, about 60 seconds each. Keep it fast. The goal is momentum, not a polished presentation.",
    // 14. Next week
    "No new brief. The only work is to implement today's critique. Refine, don't restart, no new screens or features. Come back with a before and after of what you changed and why.",
  ],
};
